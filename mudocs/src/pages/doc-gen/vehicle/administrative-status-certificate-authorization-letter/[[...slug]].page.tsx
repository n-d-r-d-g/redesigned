...
function Fields() {
  ...
  const shallowUpdateURL = useCallback(
    (authorizationMode: string) =>
      silentlyUpdateURL(
        `/doc-gen/vehicle/administrative-status-certificate-authorization-letter/${authorizationMode}`,
        router,
      ),
    [router],
  );
...
function Preview() {
  ...
  const renderBody = useCallback(() => {
    ...
    const i18nKey = <Mode extends string>(mode: Mode) =>
      `administrativeStatusCertificateAuthorizationLetter.pagePreviewBody.${mode}` as const;
...
export default function AdministrativeStatusCertificateificateificateificateAuthorizationLetter({
  authorizationMode,
}: Props) {
  const { t: tCommon } = useTranslation(DEFAULT_I18N_NAMESPACE);
  ...
  return (
    <PageWrapper<Schema>
      pageTitle={tDocGenVehicle(
        "administrativeStatusCertificateAuthorizationLetter.title",
      )}
