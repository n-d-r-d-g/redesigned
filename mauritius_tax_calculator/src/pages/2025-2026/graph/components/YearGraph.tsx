...
import { YearlyFormValues } from "../../types";
import {
...
export const YearGraph = memo(() => {
  ...
  const monthData = useMemo(() => {
    ...
    const minTotalBaseSalary = 0;
    const maxTotalBaseSalary = 13_000_000;
    const totalBaseSalaryIncrement = 100_000;
    const data: any[] = [];
    let totalBaseSalary = minTotalBaseSalary;

    for (
      ;
      totalBaseSalary <= maxTotalBaseSalary;
      totalBaseSalary += totalBaseSalaryIncrement
    ) {
      const { newPAYE, newCSG, newNSF, newFSC, newIncomeAfterTaxes } =
        retrieveYearlyValues({
          values: {
            ...values,
            totalBaseSalary: values.totalBaseSalary + totalBaseSalary,
            monthlyBaseSalary: new Decimal(values.monthlyBaseSalary)
              .add(new Decimal(totalBaseSalary).dividedBy(13))
              .toNumber(),
          },
          tCurrentYear,
        });
      data.push({
        [tCommon("graph.chargeableIncome")]: totalBaseSalary,
        [tCommon("graph.paye")]: newPAYE.round().toNumber(),
        [tCommon("graph.csg")]: newCSG.round().toNumber(),
        [tCommon("graph.nsf")]: newNSF.round().toNumber(),
        [tCommon("graph.fsc")]: newFSC.round().toNumber(),
